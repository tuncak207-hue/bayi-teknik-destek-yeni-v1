...
class _TrainingCenterScreenState extends State<TrainingCenterScreen> {
  ...
  void initState() {
    ...
    _load();
    _dio
        .post('/notifications/mark-category-read/training')
  ...
  Future<void> _open(dynamic content) async {
    if (content['type'] == 'VIDEO') {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => _VideoPlayerScreen(title: content['title'], url: url),
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('Doküman açılamadı.')));
      }
    } finally {
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  const Text('Eğitim Merkezi', style: AppText.screenTitle),
