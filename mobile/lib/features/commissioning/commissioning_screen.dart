...
class CommissioningListScreen extends StatefulWidget {
  ...
  @override
  State<CommissioningListScreen> createState() =>
      _CommissioningListScreenState();
}
...
class _CommissioningListScreenState extends State<CommissioningListScreen> {
  ...
  Future<void> _openDetail(dynamic report) async {
    final updated = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => CommissioningFormScreen(existingReport: report),
      ),
    );
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const Text('Devreye Alma', style: AppText.screenTitle),
...
class CommissioningFormScreen extends StatefulWidget {
  ...
  @override
  State<CommissioningFormScreen> createState() =>
      _CommissioningFormScreenState();
}
...
class _CommissioningFormScreenState extends State<CommissioningFormScreen> {
  ...
  late final _siteNameController = TextEditingController(
    text: widget.existingReport?['siteName'] ?? '',
  );
  late final _brandController = TextEditingController(
    text: widget.existingReport?['panelBrand'] ?? '',
  );
  late final _modelController = TextEditingController(
    text: widget.existingReport?['panelModel'] ?? '',
  );
  late final _notesController = TextEditingController(
    text: widget.existingReport?['notes'] ?? '',
  );
  late final _customerController = TextEditingController(
    text: widget.existingReport?['customerName'] ?? '',
  );
  ...
  Future<void> _loadTemplate() async {
    if (_isEditing) {
      setState(() {
        _items = (widget.existingReport['items'] as List)
  ...
  Future<void> _submit() async {
    if (_siteNameController.text.trim().isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Lütfen saha adı, marka ve model bilgilerini girin.'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (padState != null && padState.hasSignature) {
        ...
        if (bytes != null) {
          ...
          await _dio.post(
            '/commissioning/reports/$reportId/signature',
            data: formData,
          );
        }
      }
  ...
  Future<void> _deleteReport() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text(
          'Bu devreye alma raporunu kalıcı olarak silmek istediğinize emin misiniz?',
        ),
        actions: [
          TextButton(
    ...
    try {
      ...
      final file = await DocumentPdfExporter.build(
        ...
        checklist: _items
            .map(
              (i) =>
                  (label: i['label'] as String, checked: i['checked'] == true),
            )
            .toList(),
  ...
  Widget build(BuildContext context) {
    ...
    final hasSavedSignature =
        _isEditing && widget.existingReport['signatureUrl'] != null;
    ...
    if (Platform.isIOS) {
      return CupertinoPageScaffold(
        ...
        navigationBar: CupertinoNavigationBar(
          ...
          border: null,
          middle: Text(
            _isEditing ? 'Devreye Alma Raporu' : 'Yeni Devreye Alma Raporu',
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '$_checkedCount / ${_items.length} madde tamamlandı',
                            style: TextStyle(
                              color: Colors.grey.shade500,
                              fontSize: 12,
                            ),
                  ...
                  child: AnimatedContainer(
                    ...
                    child: Material(
                      ...
                      child: InkWell(
                        borderRadius: BorderRadius.circular(16),
                        onTap: () =>
                            setState(() => _items[index]['checked'] = !checked),
                        child: Padding(
                          padding: const EdgeInsets.symmetric(
              ...
              PremiumFormSection(
                ...
                children: [
                  PremiumField(
                    ...
                    label: 'Sahada dikkat çeken bir durum var mı?',
                    ...
                    if (hasSavedSignature) ...[
                      ...
                      Row(
                        children: [
                          ...
                          Text(
                            'Bu raporda kayıtlı bir imza var.',
                            style: TextStyle(
                              fontSize: 11.5,
                      ...
                      const SizedBox(height: 6),
                      Text(
                        'Yeniden imza almak isterseniz aşağıya çizin — kaydedince öncekinin yerine geçer.',
                        style: TextStyle(
                          fontSize: 11,
                          color: Colors.grey.shade500,
                        ),
