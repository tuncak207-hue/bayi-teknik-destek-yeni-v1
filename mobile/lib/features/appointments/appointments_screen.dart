...
class _AppointmentsScreenState extends State<AppointmentsScreen> {
  ...
  void initState() {
    ...
    _load();
    _dio
        .post('/notifications/mark-category-read/appointments')
  ...
  Future<void> _cancel(dynamic appointment) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text(
          '"${appointment['subject']}" randevusunu iptal etmek istediğinize emin misiniz?',
        ),
        actions: [
          TextButton(
  ...
  Future<void> _delete(dynamic appointment) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text(
          '"${appointment['subject']}" randevusu kalıcı olarak silinecek. Emin misiniz?',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(dialogContext, false),
  ...
  Future<void> _editDateTime(dynamic appointment) async {
    final currentStart =
        DateTime.tryParse(appointment['preferredStart'] ?? '') ??
        DateTime.now();
    ...
    final date = await showDatePicker(
      context: context,
      initialDate: currentStart.isAfter(DateTime.now())
          ? currentStart
    ...
    try {
      await _dio.patch(
        '/appointments/${appointment['id']}/own',
        data: {'preferredStart': newDateTime.toIso8601String()},
      );
      _load();
      if (mounted) {
      ...
      if (e.response?.statusCode == 409 && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Bu saat az önce başka bir bayi tarafından alındı. Lütfen tekrar deneyin.',
            ),
          ),
        );
      }
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const Text('Randevularım', style: AppText.screenTitle),
    ...
    try {
      ...
      if (e.response?.statusCode == 409 && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Bu saat az önce başka bir bayi tarafından alındı. Lütfen başka bir saat seçin.',
            ),
          ),
        );
            ...
            children: [
              SegmentedButton<String>(
                segments: const [
                  ButtonSegment(
                    value: 'REMOTE',
                    label: Text('Telefon/Görüntülü'),
                    icon: Icon(Icons.call_outlined),
                  ),
                  ButtonSegment(
                    value: 'ON_SITE',
                    label: Text('Sahada Ziyaret'),
              ...
              TextField(
                ...
                decoration: const InputDecoration(
                  labelText: 'Konu',
                  hintText: 'Örn: MA8000 devreye alma desteği',
                ),
              ),
