...
class _MaintenanceScreenState extends State<MaintenanceScreen> {
  ...
  Future<void> _openDetail(dynamic record) async {
    final updated = await Navigator.push<bool>(
      context,
      MaterialPageRoute(builder: (_) => _MaintenanceDetailScreen(record: record)),
    );
...
class _MaintenanceDetailScreenState extends State<_MaintenanceDetailScreen> {
  ...
  late final _systemDescription = TextEditingController(text: widget.record['systemDescription'] ?? '');
  ...
  Future<void> _save() async {
    ...
    try {
      await _dio.patch('/maintenance/records/${widget.record['id']}', data: {
      ...
      body: ListView(
        ...
        children: [
          ...
          if (widget.record['signatureUrl'] != null)
            Container(
              padding: const EdgeInsets.all(AppSpacing.sm),
              decoration: BoxDecoration(color: Colors.green.withValues(alpha: 0.06), borderRadius: BorderRadius.circular(AppRadius.sm)),
              child: Row(
                children: [
...
class _CreateMaintenanceScreen extends StatefulWidget {
  ...
  @override
  State<_CreateMaintenanceScreen> createState() => _CreateMaintenanceScreenState();
}
...
class _CreateMaintenanceScreenState extends State<_CreateMaintenanceScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      final res = await _dio.post('/maintenance/records', data: {
        'siteName': _siteName.text.trim(),
        'systemDescription': _systemDescription.text.trim(),
        'notes': _notes.text.trim(),
      });
      ...
      if (padState != null && padState.hasSignature) {
        ...
        if (bytes != null) {
          ...
          await _dio.post('/maintenance/records/$recordId/signature', data: formData);
        }
      }
